                    ...
                    @if ($expenses->count() > 0)
                        @foreach ($expenses as $expense)
                            <tr>
                                ...
                                <td>S/{{ number_format((float) ($expense->amounts ?? 0) > 0 ? $expense->amounts : $expense->expensePayments->sum('amount'), 2) }}
                                </td>
                                <td>
                                    @if (((float) ($expense->amounts ?? 0)) > 0)
                                        <div class="d-flex justify-content-between">
                                            <div>
                                                @if ($expense->paymentMethod->id == 1)
                                                    Retanqueo
                                                @else
                                    ...
                                    @else
                                        @foreach ($expense->expensePayments as $payment)
                                            <div class="d-flex justify-content-between">
                                                <div>
                                                    @if (optional($payment->paymentMethod)->id == 1)
                                                        Retanqueo
                                                    @else
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control" name="date" id="date"
                                        ...
                                        @if (auth()->user()->hasRole('seller') || auth()->user()->hasRole('operations')) readonly @endif>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
        ...
        $(document).ready(function() {
            ...
            $('#images').off('change').on('change', function() {
                ...
                Array.from(input.files).forEach(function(file) {
                    const exists = selectedFiles.some(f => !f.existing && f.name === file.name && f
                        .size === file.size && f.lastModified === file.lastModified);
